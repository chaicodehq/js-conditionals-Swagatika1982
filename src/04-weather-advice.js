...
export function getWeatherAdvice(temperature, isRaining) {
  ...
  if (typeof isRaining !== "boolean" ) return "INVALID";  
  if (typeof temperature !== "number" || Number.isNaN(temperature)) return "INVALID";  


if (temperature >= 35) {
    advisory_message = "Too hot for hiking - stay indoors and hydrate";
  } 

  else if (temperature >= 25 && !isRaining) {
    advisory_message =  "Great weather for hiking - don't forget sunscreen";
  } 
  else if (temperature >= 25 && isRaining) {
    advisory_message =  "Warm but rainy - consider indoor activities";
  } 
  
  else if (temperature >= 15 && !isRaining) {
    advisory_message =  "Perfect hiking weather - enjoy the trails";
  } 
  else if (temperature >= 15 && isRaining) {
    advisory_message =  "Cool and rainy - bring waterproof gear if hiking";
  } 
  
  else if (temperature >= 5 && !isRaining) {
    advisory_message =  "Chilly - wear layers for your hike";
  } 
  else if (temperature >= 5 && isRaining) {
    advisory_message =  "Cold and wet - best to stay indoors";
  } 
  
  else if (temperature < 5){
    advisory_message = "Too cold - stay warm indoors";
  }
  else
  {
    advisory_message = `temperature IS: ${temperature}  isRaining IS: ${isRaining}`;
