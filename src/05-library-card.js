...
if (typeof memberAge !== "number" || Number.isNaN(memberAge)) {
    return { allowed: false, message: "Too young - must be at least 6 years old" };
  }
  if (typeof hasValidCard !== "boolean") {
    return { allowed: false, message: "Invalid library card - please renew at the front desk" };
  }
  if (typeof overdueBooks !== "number" || Number.isNaN(overdueBooks)) {
    overdueBooks = 0;
  }

  // Check in the exact required order (return FIRST failure)
  if (memberAge < 6) {
    return { allowed: false, message: "Too young - must be at least 6 years old" };
  }
